...
def make_simple_ds(dsfile):
    ...
    out = dsfile.replace('ds.json', '.json')
    ...
    for k1 in dskeylist:
        if k1 in EXCLUDE:
            del(ds[k1])
        if k1 == "format" and ds[k1] != "function":
            del(ds[k1])
        # print(ds[k1])

    for i in range(len(ds['source'])):
        if 'is_available' in ds['source'][i].keys() and ds['source'][i]['is_available'] == False:
            del(ds['source'][i])
        else:
            dskeylist = list(ds['source'][i].keys())
            for k1 in dskeylist:
                if k1 == "format" and ds['source'][i][k1] != "function":
                    del(ds['source'][i][k1])
...
if __name__ == "__main__":
    ...
    dsfile = "datastructure_fullannot_v0.4.7ds.json"

    EXCLUDE = ['sourcefile2','sourcefile3','datafile_path','sourcefile','use_sourcename_in_fieldname','sourcefile_path']
    EXCLUDE_FIELD = ['sourcefile','ref_column_index','alt_column_index','fieldselection']
    EXCLUDE_SOURCE = ['sourcefile','ref_column_index','alt_column_index','fieldselection','sourcefile2','sourcefile3','is_available']
